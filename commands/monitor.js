...
CREATE TABLE IF NOT EXISTS voice_schedules (\
    ...
    module.exports.restart = async (bot) => {
        ...
        for (const ts of tsss){
            ...
            db.prepare(`UPDATE text_schedules SET interval=${interval} WHERE id=${ts.id}`).run();
        }
        ...
        for (const vs of vsss){
            ...
            db.prepare(`UPDATE voice_schedules SET interval=${interval} WHERE id=${vs.id}`).run();
        }
    }
    module.exports.run = async (bot, message, args) => {
        ...
        await queryServer(host,port,stype).then(async (state) => {
            ...
            switch(channel.type){
                ...
                insert.run(host,port,stype,[channel.guild.id, channel.id],name,interval);
                ...
                insert.run(host,port,stype,channel.guild.id,channel.id,newmsg.id,interval);
                msg.edit("Now monitoring.")
                break;
                default:
